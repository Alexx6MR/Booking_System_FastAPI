...
def get_user_bookings(request: Request, user_id: int, session: SessionDep):
    ...
    for booking in bookings:
        ...
        if classroom:
            booking_info = {
                ...
                "classroom_name": classroom.name,  # Suponiendo que tienes un campo "name" en Classroom
    ...
    if 'text/html' in accept_header:
        return templates.TemplateResponse("pages/user/user_booking_page.html", {"request": request, "bookingsList": bookings_data, "is_logged_in": global_context["is_logged_in"], "time_to_integer": time_to_integer})
        
    return JSONResponse(content=bookings_data)

# Create booking
@user_router.post("/{user_id}/create")
def book_timeslot(session: SessionDep, user_id: int, classroom_id: int = Form(...), start_time: str= Form(...), end_time: str= Form(...), timeslot_ids: List[int] = Form(...),):
